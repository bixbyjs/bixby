...
exports = module.exports = function(C, comp) {
  ...
  var subdomain = '_' + service + '._' + proto;
  
  return C.create('http://i.bixbyjs.org/ns/Resolver')
    .then(function(resolver) {
      return new Promise(function(resolve, reject) {
        resolver.resolve(subdomain + '.' + resolver.domain, 'SRV', function(err, addresses) {
          if (err) {
      ...
      if (error.code == 'IMPLEMENTATION_NOT_FOUND' && error.interface == 'http://i.bixbyjs.org/ns/Resolver') {
        throw new ImplementationNotFoundError("Unable to resolve service '" + subdomain + "' needed by '" + comp.id + "'. Did you forget to install 'bixby-ns'?", error.interface);
      }
      throw error;
    });
};
